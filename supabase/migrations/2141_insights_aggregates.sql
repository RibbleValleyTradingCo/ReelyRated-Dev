...
CREATE OR REPLACE FUNCTION public.insights_format_label(value text)
RETURNS text
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE
    ...
    ELSE initcap(regexp_replace(value, '[-_]+', ' ', 'g'))
  END;
$$;
  ...
  derived AS (
    SELECT
      ...
      CASE
        ...
        WHEN lower(COALESCE(c.weight_unit, 'kg')) IN ('kg') THEN c.weight
        WHEN lower(COALESCE(c.weight_unit, '')) IN ('lb', 'lbs', 'lb_oz') THEN c.weight * 0.453592
        ELSE c.weight
      END AS weight_kg
    FROM filtered c
  ),
