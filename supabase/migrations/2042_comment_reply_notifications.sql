-- 2042_comment_reply_notifications.sql
-- Purpose: add comment_reply notification type and emit reply notifications from create_comment_with_rate_limit.

SET search_path = public, extensions;

-- Add enum value if missing (idempotent).
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1
    ...
    WHERE t.typname = 'notification_type'
      ...
      AND e.enumlabel = 'comment_reply'
  ) THEN
    ALTER TYPE public.notification_type ADD VALUE 'comment_reply';
  END IF;
END;
$$;

-- Replace create_comment_with_rate_limit with reply notifications.
...
BEGIN
  ...
  -- Notify catch owner for new comments (skip self-comments, skip if also parent, skip soft-deleted catches)
