-- 2101_normalize_weight_units_and_fix_venue_rpcs.sql
-- Normalize legacy weight_unit values and reapply hardened venue RPCs to avoid enum cast errors.

SET search_path = public, extensions;

-- Normalize legacy 'lb' values to the canonical enum label.
-- Safe to run repeatedly; subsequent runs become no-ops.
UPDATE public.catches
SET weight_unit = 'lb_oz'::public.weight_unit
WHERE weight_unit::text = 'lb';
-- If other unexpected values ever appear, consider extending this UPDATE with additional mappings.

-- Recreate venue recent catches RPC with defensive weight_unit mapping.
...
BEGIN
  ...
  SELECT
    ...
    c.weight,
    CASE
      WHEN c.weight_unit = 'kg' THEN 'kg'::public.weight_unit
      WHEN c.weight_unit IN ('lb', 'lb_oz') THEN 'lb_oz'::public.weight_unit
...
-- Recreate venue top catches RPC with defensive weight_unit mapping.
