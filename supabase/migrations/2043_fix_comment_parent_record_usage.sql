-- 2043_fix_comment_parent_record_usage.sql
-- Purpose: guard use of v_parent to avoid unassigned record errors for top-level comments while preserving notification rules.
...
BEGIN
  ...
  -- Determine whether to notify owner (avoid double-notifying when owner is also parent)
  IF v_catch.user_id IS NOT NULL AND v_catch.user_id <> v_user_id AND v_catch.deleted_at IS NULL THEN
    IF p_parent_comment_id IS NULL THEN
      v_notify_owner := true;
    ELSE
      IF v_parent_user_id IS NULL OR v_parent_user_id <> v_catch.user_id THEN
        v_notify_owner := true;
      END IF;
    END IF;
  END IF;
