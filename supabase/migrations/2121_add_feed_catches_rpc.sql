-- 2121_add_feed_catches_rpc.sql
-- Server-side feed filters + sorts (scope/species/sort) for infinite feed.
...
COMMENT ON FUNCTION public.get_feed_catches(integer, integer, text, text, text, text, uuid, uuid) IS
  'Feed catches with server-side scope/species/sort. Privacy enforced by RLS.';

GRANT EXECUTE ON FUNCTION public.get_feed_catches(integer, integer, text, text, text, text, uuid, uuid) TO anon, authenticated;
