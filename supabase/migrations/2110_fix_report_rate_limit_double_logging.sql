-- 2110_fix_report_rate_limit_double_logging.sql
-- NOTE: We now let enforce_report_rate_limit() be the single writer to public.rate_limits
-- for action = 'reports'. create_report_with_rate_limit() still checks, but no longer logs,
-- to avoid double-counting rate-limited events.
...
-- Manual check (run in local dev):
-- 1) Clear recent rows:
-- delete from public.rate_limits
-- where user_id = '<test_user_id>' and action = 'reports';
--
-- 2) File N reports as that user via the UI/RPC.
-- 3) Confirm one row per report:
