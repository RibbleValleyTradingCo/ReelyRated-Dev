...
BEGIN
  ...
  aggregates AS (
    SELECT
      ...
      (SELECT ta.total_catches_all FROM total_all ta) AS total_catches_all,
      (SELECT d.weight FROM derived d WHERE d.weight_kg IS NOT NULL ORDER BY d.weight_kg DESC NULLS LAST LIMIT 1) AS pb_weight,
      (SELECT d.weight_unit::text FROM derived d WHERE d.weight_kg IS NOT NULL ORDER BY d.weight_kg DESC NULLS LAST LIMIT 1) AS pb_weight_unit,
