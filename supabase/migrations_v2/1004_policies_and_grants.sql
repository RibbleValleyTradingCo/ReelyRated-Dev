...
BEGIN
  -- Profiles
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'profiles_select_all') THEN
    CREATE POLICY profiles_select_all ON public.profiles
      FOR SELECT USING (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'profiles_update_self') THEN
    CREATE POLICY profiles_update_self ON public.profiles
      FOR UPDATE USING (auth.uid() = id);
  END IF;

  -- Sessions
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'sessions_select_own') THEN
    CREATE POLICY sessions_select_own ON public.sessions
      FOR SELECT USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'sessions_modify_own') THEN
    CREATE POLICY sessions_modify_own ON public.sessions
      FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
  END IF;

  -- Catches
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'catches_public_read') THEN
    CREATE POLICY catches_public_read ON public.catches
      FOR SELECT USING (deleted_at IS NULL AND visibility = 'public');
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'catches_owner_all') THEN
    CREATE POLICY catches_owner_all ON public.catches
      FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
  END IF;

  -- Comments
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'catch_comments_public_read') THEN
    CREATE POLICY catch_comments_public_read ON public.catch_comments
      FOR SELECT USING (deleted_at IS NULL);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'catch_comments_owner_all') THEN
    CREATE POLICY catch_comments_owner_all ON public.catch_comments
      FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
  END IF;

  -- Reactions
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'catch_reactions_owner_all') THEN
    CREATE POLICY catch_reactions_owner_all ON public.catch_reactions
      FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
  END IF;

  -- Ratings
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'ratings_owner_all') THEN
    CREATE POLICY ratings_owner_all ON public.ratings
      FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
  END IF;

  -- Follows
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'profile_follows_owner_all') THEN
    CREATE POLICY profile_follows_owner_all ON public.profile_follows
      FOR ALL USING (auth.uid() = follower_id) WITH CHECK (auth.uid() = follower_id);
  END IF;

  -- Notifications
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'notifications_recipient_only') THEN
    CREATE POLICY notifications_recipient_only ON public.notifications
      FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
  END IF;

  -- Reports
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'reports_owner_all') THEN
    CREATE POLICY reports_owner_all ON public.reports
      FOR ALL USING (auth.uid() = reporter_id) WITH CHECK (auth.uid() = reporter_id);
  END IF;

  -- Rate limits
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'rate_limits_owner_all') THEN
    CREATE POLICY rate_limits_owner_all ON public.rate_limits
      FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
  END IF;

  -- Lookups (readable by all)
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'baits_select_all') THEN
    CREATE POLICY baits_select_all ON public.baits FOR SELECT USING (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'tags_select_all') THEN
    CREATE POLICY tags_select_all ON public.tags FOR SELECT USING (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'water_types_select_all') THEN
