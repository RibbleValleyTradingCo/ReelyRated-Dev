...
-- GRANTS-LIVE.sql
-- Live privilege snapshot for roles of interest across non-system schemas.
-- Roles of interest (grantees): anon, authenticated, PUBLIC, service_role (if present).
--
-- NOTES
-- - `PUBLIC` is a pseudo-role (not a real row in pg_roles), so it cannot be used with has_*_privilege().
-- - To include PUBLIC privileges reliably, this script uses pg_catalog ACLs via aclexplode() for
--   schema and sequence privileges, and information_schema views for table/column privileges.
-- - This script emits ONE unified result set so shared CTEs apply everywhere.
...
order by schema_name, object_type, object_name nulls first, grantee, privilege_type;