...
-- GRANTS-REDFLAGS-PLATFORM-MANAGED-COUNTS.sql
-- Aggregated red-flag counts for platform-managed schemas (non-app surface).
-- Returns two result sets: grouped counts, then a total row count.
-- CTEs are per-statement in Postgres; the total query repeats the CTE stack.
-- Prefer split files for Supabase CSV exports:
