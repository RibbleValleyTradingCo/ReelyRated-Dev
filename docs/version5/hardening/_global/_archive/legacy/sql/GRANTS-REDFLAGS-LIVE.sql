...
-- Focused red-flag grant patterns. PUBLIC is handled via ACL grantee oid 0.
-- Avoids information_schema.schema_privileges and information_schema.sequence_privileges.
-- NOTE: This query is intentionally noisy across all non-system schemas.
-- Use GRANTS-REDFLAGS-PUBLIC-ONLY.sql for P0 validation and GRANTS-REDFLAGS-PLATFORM-MANAGED.sql for platform tracking.
...
(
  ...
  and (
    ...
    -- Optional: uncomment if you specifically want to hunt accidental public reads in app schema.
    -- or (object_type = 'table' and schema_name = 'public' and privilege_type = 'SELECT')
  )
)
or (
