...
const statIconClasses = "flex h-10 w-10 items-center justify-center rounded-full bg-primary/10 text-primary";
  ...
  return (
    <section className="relative -mt-8 space-y-3 rounded-3xl border border-border bg-card/90 p-4 shadow-card ring-1 ring-border/50 sm:p-6">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-foreground">Angler stats</h2>
      </div>
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {statsCards.map((card) => (
          <div
            key={card.label}
            className="rounded-xl border border-border bg-card/80 p-5 shadow-card transition hover:bg-card hover:shadow-card-hover"
          >
            ...
            {card.hint ? <p className="mt-3 text-xs text-muted-foreground">{card.hint}</p> : null}
          </div>
        ))}
      </div>
    </section>
